...
class Clock {
  constructor (setTimer){
    this.timer = setTimer;
    this.min = Math.floor (this.timer / 3600);
...
var buildStockLvl1 = [
  ...
  new Boat (490, 310, 0, 0),
  new Boat (490, 220, 0, 0),
  new Boat (650, 220, 0, 0),
  new Boat (270, 100, 0, 90),
...
function drawHud() {
  var posX = boat1.x + 100;
  var posYth = boat1.y +50;
  var posYan = boat1.y + 70;
  ctx.fillStyle ="white";
  ctx.font = "25px arial";
  ctx.fillText ("Timer: " + (Math.floor(clock1.timer / 60)), 900, 50);

  if (posX < 930){
    ctx.fillText ("Thrust = " + boat1.setSpeed, posX, posYth);
    if (boat1.setAngle > 0)
      {ctx.fillText ("Starboard = " + boat1.setAngle, posX , posYan);
    }

    else if (boat1.setAngle < 0)
      {ctx.fillText ("Larboard = " + (-1) * boat1.setAngle, posX , posYan);
    }

    else {ctx.fillText ("No Angle", posX , posYan);
    }
  }
